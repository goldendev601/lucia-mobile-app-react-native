...
import { sendValidationToken, updateEmail } from '../actions/luciaAppAction';
...
interface IVerificationRecoveryEmailInfoScreenProps {
    navigation: NavigationProp<any>;
}

const CELL_COUNT = 6;

const VerificationRecoveryEmailInfoScreen: React.FC<IVerificationRecoveryEmailInfoScreenProps> = (props) => {
    const dispatch = useDispatch();
    const { navigation } = props;
    const bgColor = useThemeColor({}, 'background');

    const { email } = useSelector((state: IRootState) => state.auth.accountInfo);
    const { newEmailInfo, isUpdatedEmail } = useSelector((state: IRootState) => state.luciaApp);
    ...
    const gotoUpdatePassword = () => {
        if (newEmailInfo) {
            dispatch(updateEmail(
                newEmailInfo.currentEmail,
                newEmailInfo.newEmail,
                value
            ));
        }
        if (isUpdatedEmail) {
            ...
            >
                ...
                <View style={styles.container}>
                    ...
                    <View style={styles.titleWrapper}>
                        <Text style={[styles.title, {fontFamily: 'Cormorant'}]}>Enter Code</Text>
                    </View>
                    ...
                    <Pressable style={styles.signupButton} onPress={sendVerification}>
                        <Text style={[styles.signupButtonText, {fontFamily: 'Montserrat'}]}>Resend code</Text>
                    </Pressable>

                    <Pressable style={styles.signinButton} onPress={gotoUpdatePassword}>
                        <Text style={[styles.signupButtonText, {fontFamily: 'Montserrat'}]}>Verify code</Text>
                    </Pressable>
...
export default VerificationRecoveryEmailInfoScreen;
